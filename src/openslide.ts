import Module from "./dist/openslide-api";
...
export class OpenSlideImage {
  ...
  async readRegion(
    ...
    const args = this.wasmApi._malloc(40);
    this.wasmApi.HEAP64[args / 8] = BigInt(x); // int64_t x (8 bytes)
    this.wasmApi.HEAP64[args / 8 + 1] = BigInt(y); // int64_t y (8 bytes)
    this.wasmApi.HEAP32[args / 4 + 4] = level; // int32_t level (4 bytes)
    this.wasmApi.HEAP32[args / 4 + 5] = 0; // Padding (4 bytes, required for alignment)
    this.wasmApi.HEAP64[args / 8 + 3] = BigInt(w); // int64_t w (8 bytes)
    this.wasmApi.HEAP64[args / 8 + 4] = BigInt(h); // int64_t h (8 bytes)
    this.wasmApi.HEAP32[args / 4 + 10] = readRgba ? 1 : 0; // int32_t read_rgba (4 bytes)
    ...
    this.wasmApi._free_result(args);
    const sz = w * h * 4;
  ...
  async drawToCanvas(
    ...
    this.wasmApi.HEAP64[args / 8 + 3] = BigInt(w); // int64_t w (8 bytes)
    this.wasmApi.HEAP64[args / 8 + 4] = BigInt(h); // int64_t h (8 bytes)
    this.wasmApi.HEAP32[args / 4 + 10] = 0; // int32_t read_rgba (4 bytes)
...
export default class OpenSlide {
  ...
  async initialize() {
    return new Promise<unknown>(async (resolve) => {
      const windowHandle = window as any;
      if (!windowHandle.wasmApiPromise) {
        windowHandle.wasmApiPromise = Module();
      }
      this.wasmApi = await windowHandle.wasmApiPromise;
      resolve(true);
    });
  }
