...
beforeAll(async () => {
  ...
  page.goto("http://localhost:8080/tests/test.html");
  await new Promise((resolve) => setTimeout(resolve, 3000));
}, 10000);
...
describe("Test OpenSlide (WASM file system)", () => {
  it("should import openslide class", async () => {
    const result = await page.evaluate(() => {
      return window.OpenSlide !== undefined && window.OpenSlide !== null;
    });
  ...
  it("should load the property names and values", async () => {
    const [names, value, levelCount] = await page.evaluate(async () => {
      const names = await window.slide_local_fs.getPropertyNames();
      const value = await window.slide_local_fs.getPropertyValue(names[0]);
      const levelCount = await window.slide_local_fs.getLevelCount();
  ...
  it("should have the right dimensions", async () => {
    const [level0] = await page.evaluate(async () => {
      const level0 = await window.slide_local_fs.getLevelDimensions(0);
      return [level0];
    });
  ...
  it("should have the right dimensions", async () => {
    const byteLength = await page.evaluate(async () => {
      const regionData = await window.slide_local_fs.readRegion(
        0,
        0,
        0,
        512,
  ...
  it("should close the file without error", async () => {
    const result = await page.evaluate(async () => {
      await window.slide_local_fs.close();
      return true;
    });
    expect(result).toBe(true);
  });
...
describe("Test OpenSlide (HTTP file system)", () => {
  ...
  it("should have the right dimensions", async () => {
    const [level0] = await page.evaluate(async () => {
      const level0 = await window.slide_remote_fs.getLevelDimensions(0);
      return [level0];
    });
  ...
  it("should read the right amount of data from region", async () => {
    const byteLength = await page.evaluate(async () => {
      const regionData = await window.slide_remote_fs.readRegion(
        0,
        0,
        0,
        512,
