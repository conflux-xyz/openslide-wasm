...
EMSCRIPTEN_KEEPALIVE
uint32_t* read_region(char *ptr, void* args) {
...
EMSCRIPTEN_KEEPALIVE
const char *const * get_property_names(char* ptr) {
    return openslide_get_property_names((openslide_t*) ptr);
}

EMSCRIPTEN_KEEPALIVE
const char* get_property_value(char* ptr, char* property) {
