...
if (ENVIRONMENT_IS_NODE) {
  ...
  if (numericVersion < 160400) {
    throw new Error(
      "This emscripten-generated code requires node v16.04.4.0 (detected v" +
        nodeVersion +
        ")",
    );
  }
...
function ffi_call_js(cif, fn, rvalue, avalue) {
  ...
  switch (rtype_id) {
    ...
    case 12:
      HEAPU32[(rvalue >> 2) + 0] = result;
      HEAPU32[(rvalue >> 2) + 1] = getTempRet0();
      break;
    case 15:
      throw new Error("complex ret marshalling nyi");
    default:
...
function createLegalizerWrapper(trampoline, sig) {
  ...
  var sections = [];
  var prelude = [0x00, 0x61, 0x73, 0x6d, 0x01, 0x00, 0x00, 0x00];
  sections.push(prelude);
  var wrappersig = [
    sig[0].replace("j", "i"),
    sig.slice(1).replace(/j/g, "ii"),
  ].join("");
  var typeSectionBody = [0x03];
  generateFuncType(wrappersig, typeSectionBody);
  generateFuncType(sig, typeSectionBody);
  generateFuncType("i", typeSectionBody);
  var typeSection = [0x01];
  uleb128Encode(typeSectionBody.length, typeSection);
  typeSection.push.apply(typeSection, typeSectionBody);
  sections.push(typeSection);
  var importSection = [
    0x02, 0x0d, 0x02, 0x01, 0x65, 0x01, 0x72, 0x00, 0x02, 0x01, 0x65, 0x01,
    0x66, 0x00, 0x00,
  ];
  sections.push(importSection);
  var functionSection = [0x03, 0x02, 0x01, 0x01];
  sections.push(functionSection);
  var exportSection = [0x07, 0x05, 0x01, 0x01, 0x66, 0x00, 0x02];
  sections.push(exportSection);
  var convert_code = [];
  convert_code.push(0x00);
  function localGet(j) {
    convert_code.push(0x20);
    uleb128Encode(j, convert_code);
  }
  for (var i = 1; i < sig.length; i++) {
    if (sig[i] == "j") {
      localGet(i - 1);
      convert_code.push(0xa7);
      localGet(i - 1);
      convert_code.push(0x42, 0x20, 0x88, 0xa7);
    } else {
      localGet(i - 1);
    }
  }
  convert_code.push(0x10, 0x01);
  if (sig[0] === "j") {
    convert_code.push(0xad, 0x10, 0x00, 0xac, 0x42, 0x20, 0x86, 0x84);
  }
  convert_code.push(0x0b);
  var codeBody = [0x01];
  uleb128Encode(convert_code.length, codeBody);
  codeBody.push.apply(codeBody, convert_code);
  var codeSection = [0x0a];
  ...
  function trampoline() {
    ...
    if (!ret_by_arg) {
      switch (sig[0]) {
        ...
        case "j":
          setTempRet0(HEAPU32[(ret_ptr >> 2) + 1]);
          return HEAPU32[(ret_ptr >> 2) + 0];
        case "d":
...
Module["FS_open"] = FS.open;
Module["FS_write"] = FS.write;
Module["FS_close"] = FS.close;
...
var _load_image = (Module["_load_image"] = createExportWrapper(
  "load_image",
  1,
));
...
var _close_image = (Module["_close_image"] = createExportWrapper(
  "close_image",
  1,
));
var _get_level_count = (Module["_get_level_count"] = createExportWrapper(
  "get_level_count",
  1,
));
var _get_level_downsample = (Module["_get_level_downsample"] =
  createExportWrapper("get_level_downsample", 2));
var _get_icc_profile_size = (Module["_get_icc_profile_size"] =
  createExportWrapper("get_icc_profile_size", 1));
var _read_icc_profile = (Module["_read_icc_profile"] = createExportWrapper(
  "read_icc_profile",
  2,
));
var _get_best_level_for_downsample = (Module["_get_best_level_for_downsample"] =
  createExportWrapper("get_best_level_for_downsample", 2));
var _get_level_dimensions = (Module["_get_level_dimensions"] =
  createExportWrapper("get_level_dimensions", 2));
var _free_result = (Module["_free_result"] = createExportWrapper(
  "free_result",
  1,
));
var _read_region = (Module["_read_region"] = createExportWrapper(
  "read_region",
  6,
));
var _get_property_names = (Module["_get_property_names"] = createExportWrapper(
  "get_property_names",
  1,
));
var _get_property_value = (Module["_get_property_value"] = createExportWrapper(
  "get_property_value",
  2,
));
