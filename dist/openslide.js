...
class OpenslideImage {
    constructor(wasmApi, imgPtr, filename) {
        this.imgPtr = imgPtr;
    ...
    readRegion(x, y, level, w, h, readRgba = false) {
        const data = this.wasmApi._read_region(this.imgPtr, BigInt(x), BigInt(y), level, BigInt(w), BigInt(h), readRgba);
        const sz = w * h * 4;
    ...
    drawToCanvas(canvas, x, y, level, w, h) {
        ...
        const data = this.wasmApi._read_region(this.imgPtr, BigInt(x), BigInt(y), level, BigInt(w), BigInt(h));
        const sz = w * h * 4;
...
function cstring(wasmApi, str) {
    ...
    chunk.set(str_raw);
    let terminator = wasmApi.HEAPU8.subarray(ptr + str_raw.length, ptr + str_raw.length + 1);
    terminator.set(0);
    return ptr;
}
...
class Openslide {
    ...
    async initialize(useWebworker = false) {
    ...
    async open(fileOrUrl) {
        ...
        const filename = (await loadFile(Module, file));
        const path = cstring(this.wasmApi, filename);
        const img = this.wasmApi._load_image(path);
